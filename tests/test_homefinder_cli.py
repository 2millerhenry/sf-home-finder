...
def healthy_and_on_schedule():
    """What the app actually answers once it is settled: state "current"."""
    return {
        "ok": True,
        "app": "sf-home-finder",
        "version": "0.5.6",
        ...
        "scheduled_checking": {
            "state": "current",
            "summary": "Last completed check 2 hours ago.",
        },
    }


def test_status_succeeds_when_everything_is_fine(stub_with_python) -> None:
    """A healthy app printed a healthy report and then exited 1.

    Each line of the report is an `&&` chain that reports "nothing to print"
    as a non-zero status, and the schedule line -- the one that prints nothing
    precisely when the schedule is "current", the healthy value -- was the last
    thing the script ran. So the exit code of a perfectly well app was failure,
    which is what anything scripting or monitoring this reads.
    """
    with fake_app(healthy_and_on_schedule()) as app:
        result = run(["status"], stub_with_python, port=str(app.port))

    assert result.returncode == 0, (
        f"a healthy app reported failure: {result.returncode}\n{result.stdout}"
    )
    assert "Running 0.5.6" in result.stdout
    assert "Last completed check 2 hours ago." in result.stdout
    assert "Schedule:" not in result.stdout, "a current schedule is not worth a line"


def test_status_still_fails_when_the_app_does_not_answer(stub_with_python) -> None:
    """The counterpart: the exit code has to keep meaning something."""
    result = run(["status"], stub_with_python, port=str(free_port()))
